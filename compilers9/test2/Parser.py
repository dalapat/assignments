...
from FormalVariable import FormalVariable
...
class Parser:
    ...
    def _declarations(self):
        ...
        while (self.token_list[self.current].kind == self.kind_map["CONST"]) or \
            ...
            if self.token_list[self.current].kind == self.kind_map["CONST"]:
                ...
                self._vardecl()
            elif self.token_list[self.current].kind == self.kind_map["PROCEDURE"]:
                self._procdecl()
            else:
                pass
    ...
    def _procdecl(self):
        ...
        start = self.match("IDENTIFIER")
        formal_parameters = Scope(self.current_scope)
        local_variables = formal_parameters
        self.current_scope = local_variables
        instructions = None
        return_exp = None
        self.current_scope.insert(start, Procedure(formal_parameters, local_variables,
                                                   instructions, return_exp))
        self.match("(")
        ...
        if self.token_list[self.current].kind == self.kind_map[":"]:
            ...
            self._type()
        self.match(";")
        while self.token_list[self.current].kind == self.kind_map["VAR"]:
            self._vardecl()
        if self.token_list[self.current].kind == self.kind_map["BEGIN"]:
            self.match("BEGIN")
            instructions = self._instructions()
            proc_obj = self.program_scope.find(start)
            proc_obj.body = instructions
        if self.token_list[self.current].kind == self.kind_map["RETURN"]:
            self.match("RETURN")
            exp = self._expression()
            proc_obj = self.program_scope.find(start)
            proc_obj.return_exp = exp
        self.match("END")
        end = self.match("IDENTIFIER")
        if not start == end:
            sys.stderr.write("error: procedure name not match")
        self.match(";")
        self.current_scope = self.program_scope

    def _formals(self, proc_name):
        self._formal(proc_name)
        while self.token_list[self.current].kind == self.kind_map[";"]:
            self.match(";")
            self._formal(proc_name)

    def _formal(self, proc_name):
        id_list = self._identifier_list()
        self.match(":")
        return_type = self._type()
        proc_obj = self.program_scope.find(proc_name)
        '''if isinstance(return_type, Constant):
            insert_obj = Constant()
        elif isinstance(return_type, Variable):
            pass
        elif isinstance(return_type, Array):
            pass
        elif isinstance(return_type, Record):
            pass'''
        for id in id_list:
            if not self.current_scope.local(id):
                self.current_scope.insert(id, FormalVariable(return_type))
                proc_obj.formal_parameter_order_list.append(id)
    ...
    def _factor(self):
        ...
        if self.token_list[self.current].kind == self.kind_map["INTEGER"]:
            ...
            #if self.token_list[self.current + 1].kind == self.kind_map["("]:
            ret_obj = self.program_scope.find(self.token_list[self.current].get_token_name())
            if isinstance(ret_obj, Procedure):
                self._call()
            else:
    ...
    def _call(self):
        proc_name = self.match("IDENTIFIER")
        self.match("(")
        if self.token_list[self.current].kind == self.kind_map[")"]:
            self.match(")")
            return
        proc_obj = self.current_scope.find(proc_name)

        if not isinstance(proc_obj, Procedure):
            sys.stderr.write("error: procedure")
            self.total_error_flag = 1
            exit(1)
        exp_list = self._actuals()
        proc_obj_id_list = proc_obj.formal_parameter_order_list
        for i, exp in enumerate(exp_list):
            proc_obj.formal_parameters.symbol_table[proc_obj_id_list[i]] = exp
        self.match(")")
        return ProcedureNode(None, proc_obj)
